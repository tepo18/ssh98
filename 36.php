...
# ---------------- Paths ----------------
BASE_DIR = "/storage/emulated/0/Download/almasi98"
...
def generate_groups(proxies):
    ...
    top_fast=alive[:15]; stable=[p for p in alive if p["ping"]<150][:15]; top3=alive[:15]; all_names=[p["name"] for p in alive]
    
    return [
        {"name":"💚 Top Fast","type":"url-test","proxies":[p["name"] for p in top_fast],
         "url":"http://www.google.com/generate_204","interval":100,"tolerance":950},
        {"name":"💛 Top Stable <150ms","type":"url-test","proxies":[p["name"] for p in stable],
         "url":"http://www.google.com/generate_204","interval":10000,"tolerance":3500},
        {"name":"⚡ Top 3 AutoSwitch","type":"url-test","proxies":[p["name"] for p in top3],
         "url":"http://cp.cloudflare.com/generate_204","interval":3800,"tolerance":1500},
        {"name":"🌟Fallback Stable","type":"fallback","proxies":all_names,
         "url":"http://cp.cloudflare.com/generate_204","interval":2000},
